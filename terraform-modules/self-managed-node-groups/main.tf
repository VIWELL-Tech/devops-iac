...
data "aws_ec2_instance_type" "selected_spot" {
...
resource "aws_autoscaling_group" "eks_self_managed_node_group-spot" {
  count = "${var.create_spot_workers ? 1 : 0}"
  name = "spot-${var.eks_cluster_name}-${local.node_group_name}"
  ...
  launch_template {
    id      = aws_launch_template.eks_self_managed_nodes_spot[0].id
    version = "$Latest"
  }
...
resource "aws_launch_template" "eks_self_managed_nodes_spot" {
  count = "${var.create_spot_workers ? 1 : 0}"
  name_prefix = "spot-${var.eks_cluster_name}-${local.node_group_name}"
  description = "Amazon EKS self-managed nodes (spot)"
  ...
  instance_type          = var.instance_type_spot
  image_id               = data.aws_ami.selected_eks_optimized_ami.id
  ebs_optimized          = data.aws_ec2_instance_type.selected_spot.ebs_optimized_support == "default" ? true : false
  key_name               = var.key_name
  update_default_version = true
