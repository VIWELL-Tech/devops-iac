# Most recent optimizaed Amazon EKS AMI
data "aws_ami" "selected_eks_optimized_ami" {
  owners      = ["amazon"]
  most_recent = true

  filter {
    name   = "name"
    values = ["amazon-eks-node-${data.aws_eks_cluster.selected.version}*"]
  }

  filter {
    name   = "architecture"
    values = data.aws_ec2_instance_type.selected.supported_architectures
  }

  filter {
    name   = "virtualization-type"
    values = data.aws_ec2_instance_type.selected.supported_virtualization_types
  }

  filter {
    name   = "root-device-type"
    values = data.aws_ec2_instance_type.selected.supported_root_device_types
  }
}

#### spot ####

data "aws_ami" "selected_eks_optimized_ami_spot" {
  owners      = ["amazon"]
  most_recent = true

  filter {
    name   = "name"
    values = ["amazon-eks-node-${data.aws_eks_cluster.selected.version}*"]
  }

  filter {
    name   = "architecture"
    values = data.aws_ec2_instance_type.selected_spot.supported_architectures
  }

  filter {
    name   = "virtualization-type"
    values = data.aws_ec2_instance_type.selected_spot.supported_virtualization_types
  }

  filter {
    name   = "root-device-type"
    values = data.aws_ec2_instance_type.selected_spot.supported_root_device_types
  }
}